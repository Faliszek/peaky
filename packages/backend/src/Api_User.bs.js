...
function search(payload, context) {
  var match = JwtDecode(context.token);
  var user_id = match.user_id;
  return $$Promise.Js.map($$Promise.Js.toResult(context.db.ref("/users").once("value", undefined, undefined)), (function (res) {
                if (res.TAG) {
                  return Json.fromObject(res._0);
                }
                ...
                var data = Json.toList(res._0);
                return Belt_Array.keep(data, (function (u) {
                              if (user_id === u.id) {
                                return false;
                              }
                              if (specialization === "" && firstName === "" && lastName === "") {
                                return true;
                              }
                              if (u.specialization.includes(specialization) || u.firstName.includes(firstName) || u.lastName.includes(lastName)) {
                                return true;
                              } else {
                                return false;
                              }
...
function list(_payload, context) {
  return Chain.onFinish(Chain.get(context.db, "/users"), Json.toList);
}
