...
let toMinutes = value => {
  ...
  switch (hour, minutes) {
  | (Some(hour), Some(minutes)) => hour->int_of_string * 60 + minutes->int_of_string
  | _ => 0
  }
}
...
let computeDate = (~from_, ~to_, ~date) =>
  ...
  | Some(date) =>
    ...
    (
      date->Time.startOfDay->Time.addMinutes(from_)->Time.getTime->Js.Float.toString,
      date->Time.startOfDay->Time.addMinutes(to_)->Time.getTime->Js.Float.toString,
    )
  | None => ("", "")
  }
@react.component
let make = (
  ...
  React.useEffect2(() => {
    open Select
    let p = patients->toSelectOptions->Array.keep(p => Some(p.value) == patientId)->Array.get(0)
