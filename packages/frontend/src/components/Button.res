...
module Row = {
  @react.component
  let make = (~children) => <div className="flex justify-center "> children </div>
}
...
module CTA = {
  ...
  let make = (
    ...
    let colors = switch (disabled, type_) {
    | (true, _) => "border border-gray-400 bg-gray-400 text-white cursor-not-allowed"
    | (
        false,
        #primary,
      ) => "border border-green-400 bg-green-400  text-white hover:bg-green-300 hover:border-green-300 shadow-lg"
    | (
        false,
        #ghost,
      ) => "border border-green-400 bg-white  text-green-400 hover:text-green-300 hover:border-green-300 shadow-lg"
    }
