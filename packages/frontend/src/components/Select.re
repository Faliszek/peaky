...
external onDocumentClick: (string, clickEvent => unit) => unit =
  "addEventListener";

[@bs.send] external contains: (Dom.element, Dom.element) => bool = "contains";

[@bs.scope "window"] [@bs.val]
external onDocumentClickRemove: (string, clickEvent => unit) => unit =
  "addEventListener";

let useOutsideClick = (~el: React.ref('a), ~visible, onOutsideClick) => {
  React.useEffect2(
    () => {
...
let reducer = (state: state('a), action) => {
  switch (action) {
  | SetVisibility(visible) => {...state, visible}
...
let make =
  ...
  let filterOptions =
    ...
    | ("", _) => options
    | (search, _) =>
      options->Array.keep(o => search->Js.String.includes(o.label))
    };
  ...
  let (maxHeight, height) =
    switch (loading, noOptions, filterOptions) {
    | (true, _, _)
    | (_, true, _) => ("44px", "44px")
    | (_, _, options) =>
