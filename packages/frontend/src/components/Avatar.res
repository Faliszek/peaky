...
let make = (~avatar=?, ~firstName=?, ~lastName=?, ~size=#default, ~color=?) => {
  ...
  | (Some(avatar), _, _) =>
    <img
      ...
      className={
        open Cn
        "rounded-full shadow-md border border-gray-100" + size
      }
    />
  ...
  | (_, Some(firstName), Some(lastName)) =>
    <div
      ...
      style={ReactDOM.Style.make(~backgroundColor=color, ~filter="contrast(90%)", ())}>
      <span
        className={
          open Cn
          "text-white" + fontSize
        }>
        <Text> {firstName->Js.String.get(0) ++ lastName->Js.String.get(0)} </Text>
      </span>
    </div>
  | _ => React.null
  }
