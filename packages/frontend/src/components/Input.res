module Wrap = {
  ...
  let make = (~children, ~label=?) =>
    ...
    | Some(label) =>
      <div className="mb-6">
        <span className=" text-lg mb-1 ml-2"> <Text> label </Text> </span> children
      </div>
...
let make = (
  ...
  let style = switch zIndex {
  | Some(index) => Some(ReactDOMRe.Style.make(~zIndex=index->Js.Int.toString, ()))
  | None => None
  }
  let input = (~cn=?, ()) =>
    <input
