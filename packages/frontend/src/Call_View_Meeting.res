...
let useVideoCall = (~isPatient, ~doctorPeerId, ~patientPeerId, ~localStream, ~setRemote, ~peer) => {
  open Peer
  React.useEffect0(() => {
    if isPatient {
      peer->Peer.call(doctorPeerId, localStream)

      peer->Peer.onCall("error", _err => peer->Peer.call(doctorPeerId, localStream))

      peer->onCall("call", call => {
        call->Call.answer(localStream)
        call->Call.onStream("stream", remote =>
          remote->Js.Nullable.toOption->Option.map(r => setRemote(_ => Some(r)))->ignore
        )
      })
    } else {
      peer->Peer.call(patientPeerId, localStream)
      peer->Peer.onCall("error", _err => peer->Peer.call(patientPeerId, localStream))
...
`)
@react.component
let make = (~media, ~setMedia, ~callId, ~patientId, ~doctorId, ~localStream, ~isPatient) => {
  ...
  | _ =>
    <div className="flex items-center justify-center h-screen">
      ...
      | Some(remote) =>
        <video
          ref={ReactDOM.Ref.callbackDomRef(el => WebRTC.setVideo(el, remote, true))}
          style={ReactDOM.Style.make(~transform="scale(-1, 1)", ())}
          className={
            open Cn
            "transform rounded-xl border-2 border-gray bg-black" + remoteClassName
          }
        />
      | None => React.null
      }}
      <video
        ref={ReactDOM.Ref.callbackDomRef(el => WebRTC.setVideo(el, localStream, media.video))}
        style={ReactDOM.Style.make(~transform="scale(-1, 1)", ())}
        className={
          open Cn
          "transform rounded-xl" + localClassName
        }
      />
      ...
      <div className="fixed bottom-24 flex  gap-4">
        <Button.Round
          ...
          icon={media.audio ? <Icons.Mic size="36" /> : <Icons.MicOff size="36" />}
        />
        <Button.Round
          className="bg-white text-gray-600  hover:bg-gray-100"
          onClick={_ => setMedia(SetVideo(!media.video))}
          icon={media.video ? <Icons.Video size="36" /> : <Icons.VideoOff size="36" />}
        />
        <Button.Round
          className="bg-red-400 text-white  hover:bg-red-300"
          onClick={_ => isPatient ? setEnded(_ => true) : Router.push(Calendar)}
