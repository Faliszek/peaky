...
let make = () => {
  ...
  let (createUser, result) =
    Mutation.use(
      ~refetchQueries=[|ListQuery.refetchQueryDescription()|],
      (),
    );
  <Page
    title="Pacjenci"
    ...
    <div>
      {switch (queryUsers) {
       ...
       | {data: Some(data)} when data.patients->Array.size == 0 =>
         <NoData
           buttonText={j|Dodaj pacjenta|j}
           buttonIcon={<Icons.Plus />}
           onButtonClick={_ => setCreatorVisible(_ => true)}
           title={j|Brak danych|j}
           text={j|Kliknij przycisk poniżej aby otworzyć kreator pacjenta|j}
         />
       | {data: Some(data)} =>
    ...
    <Patient_AddPatient
      ...
      onCreate={(~firstName, ~lastName, ~phoneNumber, ~disease, ~color) =>
        createUser({firstName, lastName, phoneNumber, disease, color})
        ->Request.onFinish(
