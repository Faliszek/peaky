    ...
    let make = (~icon, ~text, ~active=false, ~view) => {
      ...
      <li
        ...
        <a
          className={
            open Cn
            "px-4 py-2 flex items-center font-medium transition-colors rounded-r-lg" + className
          }>
          icon <Text> text </Text>
        </a>
      </li>
      ...
      <div>
        ...
        <ul className="bg-white">
          ...
          <Item
            ...
            | list{"consultations", _} => true
            | list{"consultations"} => true
            | _ => false
            }}
          />
        </ul>
...
let make = (~children) => {
  ...
  React.useEffect1(() => {
    ...
    | {data: Some({me, consultations})} =>
      let incoming =
        consultations
        ->Array.reverse
        ->Array.keep(c => c.userIds->Js.Array2.find(x => x == me.id)->Option.isSome)
        ->Array.get(0)
  ...
  <div className="flex">
    ...
    | (Some(call), true, path) if path->List.get(0) != Some("consultations") =>
      <div>
        <div
