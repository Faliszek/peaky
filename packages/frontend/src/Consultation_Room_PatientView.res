...
let toSelectOptions = patients => {
  open Query
  open Select
  patients->Array.map(p => {value: p.id, label: p.firstName ++ (" " ++ p.lastName)})
}

let updateCanvas = %raw(`
...
let useSharedData = (~peer, ~userIds, ~myId, ~setPatientId, ~patientId, ~setData) => {
  open Peer
  ...
  let onDataChange = React.useCallback1(data => {
  ...
  React.useEffect0(() => {
    ...
    calls.current = userIds->Array.map(x => Obj.magic(peer->connect(x, {serialization: "json"})))
...
let initalData = (myId, userIds) =>
  userIds->Array.concat([myId])->Array.map(u => {Peer.Connection.id: u, svg: ""})
...
let make = (~callerId, ~myId, ~userIds: array<string>, ~peer) => {
  ...
  let onDataChange = useSharedData(~peer, ~userIds, ~patientId, ~setPatientId, ~myId, ~setData)
      ...
      <div className="h-full py-8 px-4 w-full">
        <div
          ...
          <div className="flex items-center">
            ...
            <Button.Nav onClick={_ => setVisible(v => !v)} className="relative">
              <span style={ReactDOM.Style.make(~color, ())}> <Icons.Edit3 /> </span>
              {visible
                ? <Color_Picker
                    value=color
        ...
        <Consultation_Room_PatientData
          ...
          onChange={(svg: string) =>
            onDataChange({
              patientId: patientId,
              canvas: data->Array.map(x => x.id == myId ? {...x, svg: svg} : x),
              updater: myId,
            })}
