...
module Checkbox = {
  ...
  let make = (~checked, ~onChange, ~disabled) => {
    ...
    <div
      className={
        open Cn
        "w-8 h-8 rounded-xl flex items-center justify-center text-white" + bg + state
      }
        ...
        <div>
          <div className="shadow-md border border-gray-200 rounded-lg px-8">
            ...
            <div className="flex w-full flex-col">
              {searchSpecialist
              ->Array.map(c =>
                <div key=c.id className="flex w-full border-t border-gray-300 items-center jus">
                  <Table.Td width="w-1/5">
                    <Checkbox
                      disabled={!c.online}
                      checked={checkedIds->Js.Array2.find(x => x == c.id)->Option.isSome}
                      onChange={_ =>
                        setCheckedIds(checked =>
                          checked->Js.Array2.find(x => x == c.id)->Option.isSome
                  ...
                  <Table.Td width="w-1/4"> <Text> c.specialization </Text> </Table.Td>
                </div>
              )
              ->React.array}
            </div>
                ...
                <Button.CTA
                  ...
                  onClick={_ =>
                    createConsultation({
                      ...
                      ~onError=_ => (),
                      ~onOk=({createConsultation: consultation}) =>
