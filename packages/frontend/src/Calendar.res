...
let makeDays = (~now) => [subDays(now, 2), subDays(now, 1), now, addDays(now, 1), addDays(now, 2)]

let makeHours = () => Array.range(hours["start"], hours["end_"])
...
module Visit = {
  ...
  let make = (
    ...
    let absoluteTop = (fromDate->Time.getHours * 60 + fromDate->Time.getMinutes) * 2
    let topFrom8 = 480 * 2
    ...
    let color = patient->Option.map(p => p.color)->Option.getWithDefault("blue")

    let first = days->Array.get(0)->Option.map(Js.Date.valueOf)
    let last = days->Array.get(4)->Option.map(Time.endOfDay)->Option.map(Js.Date.valueOf)
    ...
    let index =
      ...
      ->Js.Array2.findIndex(d =>
        d->Time.startOfDay->Js.Date.valueOf == fromDate->Time.startOfDay->Js.Date.valueOf
      )
      ->Js.Int.toFloat

    let visible = width->Option.isSome ? "opacity-100" : "opacity-0"

    let left = switch width {
    ...
    switch (first, last) {
    | (Some(first), Some(last)) if from_ > first && from_ < last =>
      <div
        ref={ReactDOM.Ref.callbackDomRef(el =>
          el
          ->Js.Nullable.toOption
          ->Option.map(el => setWidth(_ => Some(el->HTMLElement.offsetWidth)))
          ->ignore
        )}
        className={
          open Cn
        ...
        <div className="flex h-full">
            ...
            | Some({firstName, lastName, disease, color}) =>
              ...
              <div className="flex items-center">
                ...
                <div className="ml-2 flex flex-col">
                  ...
                  <span className="text-sm text-gray-400"> {disease->React.string} </span>
                </div>
              </div>
            | None => React.null
            }}
          ...
          <div className="flex flex-1 p-2  items-end justify-end h-full">
            <Button.SmallRound
              onClick={_ => {
                open Router
                push(Visit(id, userId, patientId))
              }}
              icon={<Icons.Phone />}
            />
          </div>
  ...
  <Page title="Kalendarz">
    ...
    <div className="flex border flex-wrap rounded-lg relative ">
      <div
        ...
        style={ReactDOM.Style.make(~height=rowHeight->Js.Int.toString ++ "px", ())}>
        <div className="w-24" />
        {days
        ->Array.map(d =>
          <div
      ...
      ->Array.map(h =>
        <div key={h->Js.Int.toString} className="flex w-full">
          <div
            ...
            style={ReactDOM.Style.make(~width=hourWidth->Js.Int.toString ++ "px", ())}>
            <div
              key={h->Js.Int.toString}
              className="text-center h-32 flex justify-center text-lg"
              style={ReactDOM.Style.make(~height=(rowHeight * 2)->Js.Int.toString ++ "px", ())}>
              <span className="mt-2"> {now->addHours(h)->format("HH:mm")->React.string} </span>
            </div>
          </div>
          {days
          ->Array.map(d => {
      ...
      {switch query {
      | {loading: true} =>
        <div className="absolute w-full h-full flex pt-48 justify-center bg-white opacity-80">
