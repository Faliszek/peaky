  ...
  React.useEffect0(() => {
    let call = () =>
      if streams->Array.size == userIds->Array.size {
        ...
        userIds->Array.map(userId => peer->Peer.call(userId, localStream))->ignore
      }
    ...
    peer->Peer.onCall("call", call => {
      ...
      call->Peer.Call.onStream("stream", remoteStream =>
        ...
        | Some(remoteStream) =>
          setStreams(streams =>
            if (
              streams->Array.keep(x => x["id"] == remoteStream["id"])->Array.get(0)->Option.isSome
            ) {
              streams
            } else {
...
module Video = {
  ...
  let make = (~stream) =>
    <video
      ref={ReactDOM.Ref.callbackDomRef(el => WebRTC.setVideo(el, stream, true))}
    ...
    <div>
      <div className="flex w-full h-screen  overflow-hidden">
        <div className="w-1/2 flex items-center justify-center gap-4 flex-col">
          {streams->Array.map(stream => <Video key={stream["id"]} stream />)->React.array}
        </div>
        <div className="w-1/2 flex items-center justify-center overflow-y-scroll">
          <Consultation_Room_PatientView callerId myId=dataMyId userIds=dataUserIds peer=dataPeer />
        </div>
      </div>
      <div
        ...
        <video
          ...
          ref={ReactDOM.Ref.callbackDomRef(el => WebRTC.setVideo(el, localStream, true))}
        />
        <div className="flex justify-around gap-4 absolute bottom-2 w-full">
          <Button.CallButton
            ...
            icon={media.audio ? <Icons.Mic size="16" /> : <Icons.MicOff size="16" />}
          />
          <Button.CallButton
            className="bg-white text-gray-600  hover:bg-gray-100"
            onClick={_ => setMedia(SetVideo(!media.video))}
            icon={media.video ? <Icons.Video size="16" /> : <Icons.VideoOff size="16" />}
          />
          <Button.CallButton
...
let make = (~id) => {
  ...
  | ({loading: true}, _, _) =>
    <div className="flex items-center justify-center w-full h-screen"> <Spinner /> </div>
  | ({data: Some({me, consultation: Some({userIds, callerId})})}, Some(dataPeer), Some(peer)) =>
    let myId = me.id
    let dataMyId = "data" ++ me.id

    let mediaUserIds =
      userIds->Array.concat([callerId])->Array.keep(u => u != myId)->Array.map(u => "media" ++ u)

    let dataUserIds =
      userIds->Array.concat([callerId])->Array.keep(u => u != myId)->Array.map(u => "data" ++ u)

    switch view {
    | Room =>
      <Call_View_Preroom
        stream media setMedia onJoin={_ => setView(_ => Consulting)} loading=false
      />
    | Consulting =>
      <Meeting
        dataMyId
