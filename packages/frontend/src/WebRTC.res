...
let reducer = (state, action) =>
  switch action {
  | SetAudio(audio) => {...state, audio: audio}
  | SetVideo(video) => {...state, video: video}
  }
...
module MediaDevices = {
  module Constraints = {
    ...
    @obj external make: (~audio: bool=?, ~video: bool=?, unit) => t = ""
  }
  ...
  @scope(("window", "navigator", "mediaDevices")) @val
  external getUserMedia: Constraints.t => Js.Promise.t<MediaStream.t> = "getUserMedia"
}
...
module Media = {
  ...
  module Track = {
    ...
    @set external onMute: (t, t => unit) => unit = "onmute"

    @set external onEnd: (t, t => unit) => unit = "onended"
  }

  type srcObject = {
    "id": string,
    "getTracks": @meth (unit => array<Track.t>),
    "removeTrack": @meth (Track.t => unit),
  }

  type event = {"stream": srcObject}

  type trackEvent = {"streams": array<srcObject>, "track": Track.t}

  @val @scope(("window", "navigator", "mediaDevices"))
  external getUserMedia: mediaOptions => Js.Promise.t<srcObject> = "getUserMedia"
}
