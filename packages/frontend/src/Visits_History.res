...
let make = (~calls: array<Call.t>, ~callMode) =>
  <div className="shadow-md border border-gray-200 rounded-lg px-8">
    <div className="flex w-full">
      <Table.Th text=`Numer ID` width="w-1/6" />
      <Table.Th text=`Czas trwania` width="w-1/5" />
      <Table.Th text=`Data rozpoczęcia` width="w-1/5" />
      <Table.Th text=`Pacjent` width="w-1/5" />
      <Table.Th text=`Szczegóły` width="w-1/3" />
    </div>
